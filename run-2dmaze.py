"""
The script to run CenRA framework on the 2DMaze environment.
"""
...
from CenRA.Networks import QNetMiniGrid
from CenRA.Networks import RAActorMiniGrid, RAQNetMiniGrid

from CenRA.utils import minigrid_env_maker


def parse_args():
    parser = argparse.ArgumentParser(description="Run CenRA framework on 2DMaze environment.")

    parser.add_argument("--exp-name", type=str, default="CenRA-2DMaze")
    ...
    parser.add_argument("--save-folder", type=str, default="./CenRA-2DMaze/")
...
def run():
    ...
    envs = [minigrid_env_maker(env_id=env_id, seed=args.seed, render=args.render) for env_id in env_ids]

    policy_agents = [
        DQNAgent(env=env, q_network_class=QNetMiniGrid, exp_name=f"{args.exp_name}-{env.unwrapped.spec.id}",
